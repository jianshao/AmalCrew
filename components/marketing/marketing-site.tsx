    ...
    <div dir={content.dir} className="min-h-screen overflow-hidden bg-[#fbfaf7] text-ink">
      ...
      <footer className="px-5 py-12 sm:px-8">
        <div className="mx-auto flex max-w-[1240px] flex-col gap-8 border-t border-stone-200 pt-8 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="text-start text-xs text-stone-500 sm:text-end"><nav className="flex flex-wrap gap-x-4 gap-y-2 font-semibold text-brand-700 sm:justify-end"><Link href={`/${lang}/pricing`}>{lang === "en" ? "Pricing" : "الأسعار"}</Link><Link href={`/${lang}/contact`}>{lang === "en" ? "Contact" : "تواصل"}</Link><Link href={`/${lang}/privacy`}>{lang === "en" ? "Privacy" : "الخصوصية"}</Link><Link href={`/${lang}/terms`}>{lang === "en" ? "Terms" : "الشروط"}</Link>{lang === "en" ? <Link href="/en/construction-timesheet-software-uae">Construction timesheet software for UAE</Link> : null}</nav><p className="mt-3">{content.footer.location}</p><p className="mt-2">© {new Date().getFullYear()} AmalCrew. {content.footer.copyright}</p></div>
        </div>
      </footer>
